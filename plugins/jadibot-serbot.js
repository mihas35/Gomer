...
if (connection === 'close') {
if (reason === 428) {
console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ Подключение (+${path.basename(pathGataJadiBot)}) Он был неожиданно закрыт. Попытка повторного подключения...\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡`))
await creloadHandler(true).catch(console.error)
}
if (reason === 408) {
console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ Подключение (+${path.basename(pathGataJadiBot)}) Он был утерян или с истекшим сроком годности. Причина: ${reason}. Пытаюсь восстановить связь...\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡`))
await creloadHandler(true).catch(console.error)
}
if (reason === 440) {
...
console.error(chalk.bold.yellow(`Ошибка 440 не может быть отправлена по адресу: +${path.basename(pathGataJadiBot)}`))
}}
...
💻 *Обозреватель:* ${mcode ? 'Убунту' : 'Хром'}
...
> Если вы часто отключаетесь, используйте \`${usedPrefix + command}\` Если проблема не устранена, вернитесь к статусу суббота.
